% Run of a AO MIMO Filter for a single AO system
% runAO_Single.m
% Date:             13.05.2019
...
try
    if 1
        ...
        temp= '/home/daniel/02_Study/03_Master/01_MasterThesis/01_Thesis/Matlab/AO/Filter/myFunctions';
        addpath(temp)
        addpath([temp,'/HelperFunctions']);
        addpath([temp,'/TurbulenceGeneration']);
...
try
    % run filter
    [shat_mat,phihat_mat,time_mat]= myKF_FullSim(A,C,Q,R,t,s,timeLimit); %Pnorm_mat
...
try
    ...
    [shat_ten,phihat_ten,time_ten]= myMIMOTensorKF_FullSim_AO(Att,Ctt1,... % Pnorm_ten
        Ctt2,Qtt,Rtt2,1:tMax,s,tol,'PrankMax',PrankMax,'SrankMax',SrankMax,...
        'cdata',cdata);
...
try
    % run filter
    [shat_ens,phihat_ens,time_ens]= myETKF_FullSim(A,C,Q,R,N,t,s,timeLimit);

...
for iN= 1:N
    ...
    for CNT= 1:length(numLenslets)
        ...
        if ~selectSysID% only for A=a*I
            % only observable part, see massioni paper
            [~,~,V1]= svd(C);
            V1= V1(:,1:end-2);
        ...
        [shat_ten,phihat_ten,temp]= myMIMOTensorKF_FullSim_AO(Att,Ctt1,Ctt2,...
            Qtt,Rtt,t,s,tol,'PrankMax',PrankMax,'SrankMax',SrankMax,...
            'cdata',cdata,'timelimit',timeLimit);
